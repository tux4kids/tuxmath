...
snprintf(xml_lesson_path, 4096, "%s/new/lessonData.xml", mission_path);
 ...
 srfc = BlackOutline(_("Searching for lesson file ..."), DEFAULT_MENU_FONT_SIZE+10, &red);
        if (srfc)
        {
...
bkgd_rect = current_bkgd()->clip_rect;
 ...
 /* Draw background shading for table: */
      table_bg.x = (screen->w) - (max_width + 65) ; //don't draw over Tux
      table_bg.y = 5;
      table_bg.w = max_width +20;
          ...
          text_rect.h = srfc->h;
           ...
           if(temp+i==selected)
             ...
             { 
               ...
               dest.x =  button_rect.x + 8 ;
               dest.y = button_rect.y +10 ;
               dest.w = images[IMG_LOCK]->w;
               dest.h = images[IMG_LOCK]->h;
               //dest.w = button_rect.w;
               //dest.h = button_rect.h;
  
               SDL_BlitSurface(images[IMG_LOCK], NULL, screen, &dest);    
             }       
