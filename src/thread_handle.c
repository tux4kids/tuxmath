...
int  start_new_thread(Uint16 temp_port)  // return 1 on success else 0. 
{
	...
	{	
		new_thread->client_socket.port=temp_port;  // allocating port number to newly created thread 
		...
		if(common_connect_server(&host_ipaddress,&new_thread->client_socket.server_socket,new_thread->client_socket.port,(const char *)NULL)==0)
		{
...
void cleanup_thread()
{
	...
	while(thread_header!=NULL)
	{
		temp_thread=thread_header;
		thread_header=thread_header->next;
		SDLNet_TCP_Close(temp_thread->client_socket.server_socket);
